...
class Robot:
    ...
    def detect_color_and_steer(self):
        r, g, b = self.color_sensor.color_rgb_bytes
        r_avg = self._color_filter.add(r)
        g_avg = self._color_filter.add(g)
        b_avg = self._color_filter.add(b)
        ...
        # Detect blue color
        blue_detected = b_avg > 50 and b_avg > r_avg and b_avg > g_avg
        # Detect orange color by strong red and some green, more than blue
        orange_detected = r_avg > 50 and r_avg > g_avg and g_avg > b_avg

        if blue_detected:
            left_angle = max(self.servo._min, 60)  # turn left
            self.servo.set_angle(left_angle)
            logger.info("Blue detected: Turning left")
        elif orange_detected:
            right_angle = min(self.servo._max, 120)  # turn right
            self.servo.set_angle(right_angle)
            logger.info("Orange detected: Turning right")
        else:
            self.servo.set_angle(90)  # go straight
            logger.info("No target color detected: Going straight")
    ...
    def main_loop(self):
        try:
            ...
            while self.lap_count < Config.TOTAL_LAPS:
                ...
                # Use color-based steering
                self.detect_color_and_steer()

                front = self.sensors.range_cm('F')

                # Motor control based on front distance
                if front < 20:
                    self.motor.stop()
