...
class LEDController:
    ...
    def blink(self, times: int, color=None, speed=0.2) -> None:
...
class SensorManager:
    ...
    def range_cm(self, name: str) -> float:
        ...
        mux = self._sensors[name]._i2c_device.i2c  # hack to reselect mux
        mux.channel_enable(ch)
...
# -----------------------------------------------------------------------------
# CAMERA SENSOR (Pi Camera Module 3 + OpenCV)
# -----------------------------------------------------------------------------
class CameraSensor:
    def __init__(self, resolution=(640,480), framerate=20):
        ...
        # ROI for line & finish detection
        h = resolution[1]
        self._line_roi      = (int(h * 0.6), h)
        self._finish_thresh = 250
    ...
    def read_frame(self):
        arr = self.picam2.capture_array(wait=True)
        return cv2.cvtColor(arr, cv2.COLOR_BGR2RGB)

    def compute_line_offset(self, frame):
        y1, y2 = self._line_roi
        gray = cv2.cvtColor(frame[y1:y2], cv2.COLOR_RGB2GRAY)
    ...
    def detect_finish_line(self, frame):
        y1, y2 = self._line_roi
        gray = cv2.cvtColor(frame[y1:y2], cv2.COLOR_RGB2GRAY)
        count = cv2.countNonZero(gray < 50)
        return (count > self._finish_thresh)
...
class Robot:
    ...
    def __init__(self) -> None:
        ...
        # Camera
        self.camera = CameraSensor(resolution=(640,480), framerate=20)
        self.camera.start()
    ...
    def shutdown(self) -> None:
        """Stop motors/LEDs, camera, and cleanup GPIO."""
    ...
    def main_loop(self) -> None:
        """Drive, avoid walls, count laps, and use camera guidance."""
        try:
            ...
            while self.lap_count < Config.TOTAL_LAPS:
                ...
                # ——— Camera-based line follow & finish detection —————————————————
                frame = self.camera.read_frame()

                # compute offset and steer
                offset = self.camera.compute_line_offset(frame)
                cam_angle = ((Config.STEER_RIGHT_DEG - Config.STEER_LEFT_DEG)
                             * (offset + 1)/2 + Config.STEER_LEFT_DEG)
                self.servo.set_angle(cam_angle)
                self.motor.forward(50)

                # blink on finish line
                if self.camera.detect_finish_line(frame):
                    logger.info("Camera detected finish line!")
                    self.button_led.blink(2)
                    time.sleep(1)

                # optional: ArUco orientation debug
                ang = self.camera.detect_marker_angle(frame)
                if ang is not None:
                    logger.debug("Camera marker angle: %.1f°", ang)

                # ——— Original ToF & color-sensor logic —————————————————————
                self.check_lap()
